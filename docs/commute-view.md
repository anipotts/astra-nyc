...
// work, stops camera, clears route. Reactivation automatically replays a valid
// route cache or schedules a fresh lookup.
// On teardown:
commute.destroy();
routeLayer.destroy();
```
...
The default `/api/location` middleware resolves explicitly submitted custom public destinations and requires a matching candidate choice. 3 World Trade Center is a finite reviewed public demo point. Typing and camera motion make no provider request. Entering active Commute with resolved endpoints, changing mode, or choosing a resolved candidate schedules the route after a 180 ms debounce. Transit immediately offers its external link. Static hosting without these APIs keeps external directions usable.
...
## Initial-slice verification receipt — September 10, 2026
...
## Automatic route follow-up — September 10, 2026

The shared mount/update contract is unchanged. `index.js` now delegates route intent to `createAutomaticRoute` in `automatic-route.js`. Active Commute with a selected listing and resolved endpoints schedules a lookup after 180 ms. Rapid mode/listing/view changes cancel the timer and invalidate in-flight client responses immediately. Identical context and priority-only updates do not schedule another request, including after an error. Mode and destination remain visit-local across listings. Re-entry may replay the existing controller cache.

The redundant Preview button is removed from normal resolved flows. Unknown destination typing invalidates the old route and exposes **Find place**; Enter or that button explicitly starts the existing geocoder. Choosing a candidate then starts routing without another submit. **Retry route** appears only after a failed route acquisition. Transit switches immediately to the existing external itinerary link without an in-app provider call. The server's profile separation, cache/coalescing, rate limit and timeout are unchanged.

Nine added offline regressions exercise the scheduler and actual mounted-view event handlers: automatic entry/mode/candidate routing, typing without calls, equivalent/priority-only update deduplication, failure-only retry, cache replay on re-entry, rapid-change cancellation, transit and late-response suppression. The view event tests use a minimal DOM adapter and ignore the CSS module; actual integrated browser validation belongs to Direction. All 141 tests on this branch passed (132 prior + 9 new); the production build passed with its existing chunk-size warning. No temporary server, new dependency, provider or model call was used for this follow-up.
