...
The interface uses the participant-approved home-to-home PNG artwork in forest green, with three desktop view tabs and the listing selector stacked below the brand. Listing facts belong directly below the selector in the sidebar; there is no bottom facts bar. The map and Inside surface retain full height. This project targets desktop only. Plans remains a native modal with keyboard focus and Escape dismissal. View changes preserve the map instance and restore the selected listing's Overview camera. Map motion is bounded to 650ms; result reveal and route emphasis are cancellable and disabled with reduced motion. No model calls run on camera movement.

Around a selected NYC location, the map loads a bounded set of official NYC OTI building footprints and recorded roof heights, converting feet above ground to metres. Unknown heights stay flat. Complete local results replace the corresponding OpenStreetMap massing; the base map remains outside that area and on acquisition failure. The OpenStreetMap tile pipeline mixes recorded heights with storey estimates; its fixed5m bucket is excluded conservatively. Neither layer establishes roof shape, facade materials, entrances, interiors or current surveyed conditions. Sources & details contains acquisition dates, varying feature edit dates, counts and coverage limits. See docs/NYC_BUILDINGS.md for the service contract.
...
The separate same-origin POST /api/plans/source retrieves only exact PDF URLs already in the approved source catalog. It validates redirects, MIME/signature and streamed size, with a4MB/15second acquisition limit, two concurrent requests and no persistent file storage. PDF.js6.3.289 renders the original pages locally, bounded to three pages and2400pixels per canvas dimension. Closing Plans cancels pending work. Unsupported documents keep an explicit publisher link. The viewer neither reconstructs geometry nor certifies dimensions, current condition, copyright reuse rights or building-code compliance. See src/source-plan/README.md for the contract.
...
The retired evaluation process used http://127.0.0.1:5175. Its frozen attempt budget and receipts remain private; do not restart it to reset that budget. The participant preview is exclusively http://127.0.0.1:5174 from canonical main. A static build does not host the API.
