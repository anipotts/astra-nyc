...
### 2026-09-10T18:15:05Z — Local illustrated landing motion

Added a standalone decorative WebGL layer over the approved landing artwork: rigid slow drift and feathered open-water brightness shimmer, retaining the original daylight palette and stationary HTML heading/search. This is animation of an illustration, not new geographic or listing geometry. A keyboard-operable Pause/Play action freezes the current frame. Input focus, leaving the landing, hidden documents and reduced-motion preference stop the animation loop. Reduced motion and GPU initialization failure retain the CSS poster. Draws are capped at30fps with a bounded backing store; the source remains1536×1024, not newly generated higher-resolution media. No service credits, uploads, live model calls or dependencies added.

Verification:112existing tests and production build pass (existing large-chunk advisory); desktop browser checked723×856 and1144×853 without page overflow. Actual screenshot pairs show changing water pixels while paused screenshots are pixel-identical. Verified input-focus pause, keyboard resume, reduced-motion poster and hidden controls, and transition to95Wall evidence-only Overview with motion inactive. Browser log API returned no errors/warnings; prior map diagnostics are outside this patch. GPU failure and background-tab scheduling are code-reviewed fallbacks, not simulated hardware-failure proof. Integration must include the separate startup/status-reset patch; this branch retains the base build's known stale Add listing message. Preview5176 was internal; main and participant5174 were untouched.
