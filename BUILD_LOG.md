...
## 2026-09-10T16:11:31-0400 — Compact sidebar and walking route integration

Merged the designer's selected-address hierarchy and the Commute walking preview into main, then connected the four walking controls to the shared map layer. The sidebar uses one section label, the selected property's sourced street address, compact facts and one availability status. The long NYC agency map credit now lives in the source disclosure; base-map attribution remains visible. The shared UI contract records the current structure.
