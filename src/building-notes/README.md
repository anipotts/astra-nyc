...
The compact inspector uses shared UI tokens and primitives. It keeps the latest complaint date, source status and agency outcome visible, with one record/source disclosure. Identical agency explanations are displayed once across issue rows; distinct explanations remain associated with their issues. A closed complaint uses a neutral badge and does not imply repair. The buildings icon consumes the shared Phosphor sprite supplied by the designer.
