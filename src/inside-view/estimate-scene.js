...
export function createEstimateScene(host, data) {
  ...
  renderer.domElement.tabIndex=0;renderer.domElement.setAttribute('aria-label','Estimated apartment in 3D. W A S D or arrow keys to move. Drag to look. Home resets the view.');
  host.append(renderer.domElement);
  const camera=new THREE.PerspectiveCamera(68,1,.035,80), mats=[],geos=[],textures=[],colliders=[];
  ...
  const texture=new THREE.CanvasTexture(textureCanvas);texture.colorSpace=THREE.SRGBColorSpace;texture.wrapS=texture.wrapT=THREE.RepeatWrapping;texture.repeat.set(.7,.7);textures.push(texture);
  const floorMat=new THREE.MeshStandardMaterial({map:texture,roughness:.7});mats.push(floorMat);
  ...
  for(const w of data.walls){
    ...
    const solid=(width,height,x,y)=>{const mesh=box(group,width,height,.12,x,y,0,plaster);colliders.push(mesh);};
  ...
  box(avatar,.36,.54,.23,0,1.05,0,dark,.08);box(avatar,.23,.27,.23,0,1.47,0,wood,.08);
  ...
  function draw(){
    avatar.position.set(x,0,z);avatar.rotation.y=yaw;
    const target=new THREE.Vector3(x,1.46,z),offset=new THREE.Vector3(.32,.24,1.35).applyAxisAngle(new THREE.Vector3(0,1,0),yaw),desired=target.clone().add(offset);
    scene.updateMatrixWorld(true);
    const ray=new THREE.Raycaster(target,offset.clone().normalize(),0,offset.length());const hit=ray.intersectObjects(colliders,false)[0];
    if(hit)desired.copy(target).addScaledVector(offset.clone().normalize(),Math.max(.12,hit.distance-.15));
    // Floor outline is a second guard against the camera crossing an exterior opening.
    while(!insideCamera(desired)&&desired.distanceTo(target)>.15)desired.lerp(target,.25);
    camera.position.copy(desired);camera.lookAt(target.clone().add(new THREE.Vector3(-Math.sin(yaw)*3,pitch*3,-Math.cos(yaw)*3)));
    renderer.render(scene,camera);
  }
  function insideCamera(p){return walkable({...data,walls:[],furniture:[]},p.x,p.z,.07);}
  function tick(time){frame=0;if(!active)return;const dt=Math.min((time-last)/1000,.04);last=time;
  ...
  frame=requestAnimationFrame(tick);draw();
  return { reset, setActive(value){active=value;keys.clear();cancelAnimationFrame(frame);if(active){last=performance.now();frame=requestAnimationFrame(tick);draw();}}, destroy(){active=false;cancelAnimationFrame(frame);abort.abort();observer.disconnect();geos.forEach(g=>g.dispose());mats.forEach(m=>m.dispose());textures.forEach(t=>t.dispose());renderer.dispose();canvas.remove();} };
}
